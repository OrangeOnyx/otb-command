...
- `npm run site-twin` → `dist-twin/OTB_Site_Twin/` (gitignored): `model.glb` (1.28 MB · 648 nodes · 6.7k tris; every node
  `extras.assetId` = A-1 register id), `twin-data.json`, `site-register.csv`, `twin-report.json`, offline viewer + vendored
  three.js. `--check` = byte-identical rebuild. Zip delivered to `G:\My Drive\00 OTB\site-twin\OTB_Site_Twin.zip`.
- 745 register assets → 616 modeled / 129 data-only (unit-centroid + unlocated rows, zones, buildings). Placeholders 0.
- D1 canopy 10'/14' + D2 one RTU proxy per suite = presentation; D3 historic trees hidden; D4 plat axes (plan view = A-1).
- Browser-verified on the 8770 preview: overview/plan/walkway tour 1→39, pick (canvas + list), zone → 56 stalls, record look,
  LUS toggle, data-only meter rows, placeholders, 375 px stacked layout. Basis: `docs/site-twin-2026-09.md`.
- `.claude/launch.json` gained `otb-site-twin-8770` (uncommitted, like `otb-review-5213`).
- NEXT (optional): load the GLB into A-2 Lens-B; field-measure canopy height to replace D1.
...
**Migrations — all three landed through Claude's own door this session (first time since Sep 1):**
- `20260911130000_hvac_contracts.sql` — APPLIED via apply_migration (the one-liner is retired). NOTE: the operator's SMK-16 PASS (Sep 17) predates the table — Add contract / Log service could not have persisted; Rev 12 adds **SMK-20** (re-run) and leaves the mark alone.
- `20260911140000_property_identifiers.sql` — owner row corrected to **Belle Realty of Lafayette, LLC** per D-23b, then APPLIED. Verified on prod: 25 identifier rows, 12 blank, owner = Belle Realty; the AC value survives only in the `source` stamp.
- `20260917120000_signin_log.sql` (NEW) — `signin_log` (operator read, no client write policy) + `log_signin` definer RPC (email from JWT, role from profile, 10-min session dedupe, 400-day retention) — APPLIED; `log_signin` present on prod.

**Built (record: `docs/parity-reconciliation-2026-09-11.md` §Rulings — 2026-09-17):**
- **D-19a** `instruments.json → titleExceptions` (all 13 AC recorded agreements verbatim; `registerDoc` links 7 to K-1 documents) · `TITLE_EXCEPTIONS` in facts.js · K-1 Document Register foot: "Recorded instruments of record · 13 · 6 of record only".
- **D-19b** `src/data/pylon.json` (14 panels, AC slot status, P13 `physicalReads: "Boulevard Nutrition"` → "reprint needed") · `tools/pylon.py` reads it (both SVGs regenerated byte-identical) · `PYLON` in facts.js · K-1 "Pylon sign register" card.
- **D-23a** `src/lib/identifiers.js` (pure fold, blanks = "— not on file") + `remote.getPropertyFacts()` · K-1 "Property identifiers" card (hosted only; local review shows a "Hosted only" line).
- **D-24b** `src/lib/signins.js` (pure KPI) + `remote.logSignin/listSignins` · boot stamps one row after the error beacon · D-1 "Sign-ins (7d)" KPI (operator read; absent when empty).
- **D-4b** `OTB_LATE_POLICY` cited to the OTB standard lease **§4.01(C)** ("$100 plus $25 per day beginning day six", `lease-body.json`); pinned in `test/register-facts.test.mjs`.
- **D-15 / D-24a** — no build by pick; D-24a (read_only → owner) folded into the H-3 card text.
- Tests: `test/identifiers.test.mjs`, `test/signins.test.mjs`, `test/register-facts.test.mjs` (+12).

**Punch list Rev 12 published** (state r30 carried verbatim; repo copy == live body, diff 0 before the rebuild): Schedule B +SMK-18 (K-1 registers) / SMK-19 (D-1 sign-in KPI) / SMK-20 (HVAC re-run); Schedule E rows 15/19/23 → DONE (16/25); Schedule F +F-25; Sep 17 note answers SMK-5 (ACH stays the only rail) and SMK-6 (L-1 Select → Select all → Delete, one confirm).

**Open on the operator now:** (1) SMK-18 / 19 / 20 on production; (2) D-4a (Aug-2026 seam) and H-3 (now carrying D-24a) on the sheet; (3) verify the F-2 cap table; (4) H0-3 secret rotation; (5) domain move F-23 + Preview env; (6) lease follow-ups. **No un-gated build work remains** — every Schedule F item left is Tier 2 (decision / hand-step) or Tier 3. Local `docs/graph/labels.json` edit still uncommitted and unrelated — left alone.
...
- **F-1 Invoices** — `src/lib/invoice.js` (INV-YYYYMM-<unit>, paid/partial/void DERIVED from the ledger FIFO, never stored), `invoiceUI.js` drawer panel (month picker · ⤓ Invoice printable with `<title>` = number so Save-as-PDF names it · operator Mark sent / Void), tenant M-1 face lists issued invoices read-only. NEW table `invoices` — **APPLIED on prod via apply_migration (in server history)**. Assert suite extended.
- **F-2 CAM reconciliation** — NEW `src/data/recovery-terms.json` (per-unit caps transcribed from the AC lease abstracts: 143 = 4% CAM-only; 101/103/105/109/111/115/119/121/123/125/127/141/145 = 5% CAM/Tax/Ins from lease year 2; 145 audit rights; **no base-year clause exists anywhere → baseYear null**; 119 + 145 have no commencement on the roll so their cap cannot apply yet). `financials.opexYears` (prior-year actuals inside the existing singleton). `camrecon.js`: per-component caps, lease-year test, derived occupancy gross-up (calc/grossup.js, 95% target; manual pct = override), per-tenant true-up; 135B absorbed like vacancy; unknown-PSF suites (143/145/117.5) sort last and print "Not determinable". NEW `camstatement.js` per-tenant annual reconciliation statement (tenant palette, PSF breakdown, cap line, methodology). P-1 card: year picker + prior-year worksheet + per-row ⤓ Statement. **Operator: verify the cap table against the signed leases** (`docs/superpowers/plans/…` §F-2 + the builder's table in this entry).
- **F-3 Report variants** — `boardReportModel/HTML(…, {audience: board|lender|stakeholder})`; S-1 card "Quarterly reports" with ⤓ Board · ⤓ Lender · ⤓ Stakeholder. Lender adds collections + aging from live reads (best-effort) and an unconditional "Debt service and covenant data are not on file" line; stakeholder adds an unconditional "Member distributions and capital accounts are not on file" line. Shared doc brand extracted to `src/lib/docbrand.js`.
- **F-4 HVAC PM contracts** — `src/lib/hvac.js` mirrors governance.js; next-due DERIVED (last service + frequency); T-1 rows "HVAC PM due — <unit> · <vendor>"; drawer HVAC block now paints `hvac_units` systems (first client seam over the 28 imported rows) + contract lines with Log service / Add contract (149 button "From §9.01" prefills Butcher Air monthly — a prefill, not a seed). NEW table `hvac_contracts` — **NOT YET ON PROD: both SQL doors classifier-blocked after the invoices apply.** Operator one-liner: Supabase SQL editor → paste `supabase/migrations/20260911130000_hvac_contracts.sql`. Until then the seam logs the missing table and T-1/drawer degrade silently (verified in code: error object, not a throw).
- **F-5 Parity reconciliation** — `docs/parity-reconciliation-2026-09-11.md` (rows #4 #15 #19 #20 #23 #24; #20 → DONE 25/25 logos; #24 stays OPEN). Closed: 2019 Broussard MAI appraisal as a repo-locked fact (`APPRAISAL_2019`, S-1 "Valuation on file", P-1 cap hint 8.50%); concierge live digest now summarises ledger aging / open M-1 / N-1 deadlines / governance due / deals / SOP overdue (PII-scrubbed); `20260911140000_property_identifiers.sql` written — **deliberately NOT applied: it would write "On The Boulevard, LLC" as owner entity (AC value) against the audit-grade Belle Realty record → waits on D-23b.** Nine operator decisions owed (D-4a/4b, D-15, D-19a/b/c, D-23a/b, D-24a/b) — forced rankings in the doc and on punch list Rev 11. Finding: AC double-assessed every Aug-2026 late fee (2 × $50) — the "$100" was a bug, not policy.
