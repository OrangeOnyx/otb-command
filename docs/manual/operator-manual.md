...
**Client Errors (24h)** (renders only when the count is nonzero), and the
...
- **📦 Lead SMS** (leasing persona): copies a ready-to-send text with the
  public leasing one-pager (orangeoceanatlas.com/leasing.html) and the
...
- 11:00 UTC — proactive scan: renewals/holdovers/occupancy → AI threads;
  ...
  unmatched-ACH threads; unbooked voice leads; UniFi outages. The scan also
...
`node --check` on changed modules · `npm test` (340) · `npm run build` ·
