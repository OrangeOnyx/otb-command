...
## Rulings — 2026-09-17 (punch list state r30, saved 2026-09-18T02:34Z)

The operator picked **option 1** on eight of the nine decisions above. Acted on
the same day; D-4a and H-3 stay open.

| Decision | Pick | Done |
|---|---|---|
| D-4a seam convention | undecided | — (Atlas ledger remains the sole Aug-2026 record by default; nothing imported) |
| D-4b late-fee policy | 1 | `OTB_LATE_POLICY` kept; source cited in `src/lib/ledger.js` = OTB standard lease **§4.01(C) Payment** ("$100 plus $25 per day beginning day six", `src/data/lease-body.json`); pinned by `test/register-facts.test.mjs` |
| D-15 read-only query tool | 1 | not built; unanswered questions log through AI-1 transcripts (already persisted) |
| D-19a five recorded instruments | 1 | `instruments.json → titleExceptions` (all 13 rows verbatim, `registerDoc` links 7 to K-1 documents); `TITLE_EXCEPTIONS` in facts.js; K-1 "Recorded instruments of record" block under the Document Register |
| D-19b pylon register | 1 | `src/data/pylon.json` (14 panels, AC slot status + the P13 "Boulevard Nutrition" reprint flag); `tools/pylon.py` reads it (SVGs regenerated byte-identical); `PYLON` in facts.js; K-1 "Pylon sign register" card |
| D-23a identifiers surface | 1 | K-1 "Property identifiers" card (`src/lib/identifiers.js` fold + `remote.getPropertyFacts()`), nulls as "— not on file", grouped as AC grouped them |
| D-23b owner entity | 1 | migration row corrected to **Belle Realty of Lafayette, LLC** before load (AC value kept only in the source stamp); `20260911140000_property_identifiers.sql` APPLIED on prod 2026-09-17 — 25 rows, 12 null |
| D-24a read-only users | 1 | read_only → owner; folds into H-3 (still the operator's) |
| D-24b sign-in audit | 1 | `20260917120000_signin_log.sql` APPLIED on prod: `signin_log` (operator read, no client write) + `log_signin` definer RPC (email from JWT, role from profile, 10-min session dedupe, 400-day retention); boot stamps one row; D-1 "Sign-ins (7d)" KPI (`src/lib/signins.js`) |

Also this session: `20260911130000_hvac_contracts.sql` APPLIED on prod (the
door opened after three blocked sessions) — SMK-16 had been marked PASS
against an absent table; the operator should re-run its Log service step.
