...
test('captured Atlas report distinguishes contractual rent, dated entries, historical claims and unknown actuals', () => {
  ...
  assert.equal(report.scheduled.asOf, '2026-09-10');
  assert.equal(report.scheduled.monthly, 88070.71);
  assert.equal(report.scheduled.annualized, 1056848.52);
  ...
  assert.equal(report.scheduled.units['143'].monthly, 3354.75);
  assert.equal(report.scheduled.units['145'].monthly, 798.75);
  assert.equal(report.scheduled.units['139'].monthly, 2955.38);
  assert.equal(report.scheduled.units['139'].leaseEvidence.plannedRenewal.monthly, 6150.38);
  assert.equal(report.scheduled.units['139'].leaseEvidence.plannedRenewal.includedInSchedule, false);
  assert.equal(report.scheduled.units['143'].leaseEvidence.ownerReportedPayment.bankReconciled, false);
  assert.ok(report.sources.some(source => source.id === 'lease-review-145'));
  assert.doesNotMatch(report.sources[0].excerpt, /matched the hash-verified deployed Atlas/);
...
test('endpoint returns 405, 401, 403 and 503 without constructing or leaking a private report', async () => {
  ...
  for (const dependencies of [{ authorize: async () => { throw new Error('secret upstream token'); } }, { authorize: async () => gate, build: () => { throw new Error('private malformed financial input'); } }]) {
    ...
    assert.equal(res.body.error, 'Ledger extract is currently unavailable.');
  }
});
...
test('endpoint refuses to publish the extract on a deployment wired to another Supabase project', async () => {
  let built = false;
  const handler = createAtlasNumbersHandler({ authorize: async () => gate, build: () => { built = true; return { private: 12345 }; }, projectRef: () => 'hefexnqkigirmzpmeggj' });
  const res = response(); await handler({ method: 'GET', headers: {} }, res);
  assert.equal(res.statusCode, 404);
  assert.equal(built, false);
  assert.ok(!JSON.stringify(res.body).includes('12345'));
  const ok = createAtlasNumbersHandler({ authorize: async () => gate, build: () => ({ private: 1 }), projectRef: () => 'kbhsghodquchkgfdzckc' });
  const res2 = response(); await ok({ method: 'GET', headers: {} }, res2);
