...
test('authenticated import rejects other-scope exports before changing state, accepts legacy unscoped exports',()=>{
  const disk=storage();configureStoreScope(scope,{storage:disk});
  setNote('101','keep');
  for (const binding of [local,{...scope,propertyId:'other'}]) {
    assert.throws(()=>importJSON(JSON.stringify({version:1,scope:binding,notes:{101:'replace'}})),/different account or property/);
  ...
  // Exports made before the scope field existed (every pre-2026-09 export and
  // Drive backup) carry no scope and remain importable — operator ruling.
  importJSON(JSON.stringify({version:1,notes:{101:'legacy'}}));assert.equal(getNote('101'),'legacy');
