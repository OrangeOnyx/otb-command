...
export function createCommandNumbers({host, account, onChange = () => {}, onSource, onInvalidate = () => {}}) {
  ...
  function unavailable(message, {retry = false, pending = false} = {}) {
    host.innerHTML = `<div class="cmd-numbers-head"><div><h2 id="cmdNumbersTitle">Rent schedule &amp; ledger records</h2><p>${esc(message)}</p></div>${retry ? '<button class="cmd-secondary" data-atlas-retry>Try again</button>' : ''}</div>
  ...
  async function load() {
    if (LOCAL_REVIEW || !REMOTE) { clear('The reviewed rent schedule and dated ledger records are private.'); return; }
    ...
    unavailable('Loading the reviewed schedule and dated ledger records.', {pending:true});
    try {
  ...
  function render() {
    ...
    const records = count => `${count || 0} ${count === 1 ? 'entry' : 'entries'} in the production ledger`;
    host.setAttribute('aria-busy','false');
    host.innerHTML = `<div class="cmd-numbers-head"><div><h2 id="cmdNumbersTitle">Rent schedule &amp; ledger records</h2><p>Rent schedule reviewed ${esc(date(scheduled.asOf))}. Production ledger captured ${esc(date(report.capturedAt))}; historical entries remain separate.</p></div>
      ${report.periods.length ? `<label class="cmd-numbers-period">Entry month<select aria-label="Ledger entry month">${[...report.periods].sort((a,b) => b.period.localeCompare(a.period)).map(row => `<option value="${esc(row.period)}"${row.period === period ? ' selected' : ''}>${esc(month(row.period))}</option>`).join('')}</select></label>` : ''}</div>
